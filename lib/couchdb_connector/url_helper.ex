defmodule Couchdb.Connector.UrlHelper do
  ...
  defp do_database_server_url(db_props = %{user: nil, port: nil}) do
    "#{db_props[:protocol]}://#{db_props[:hostname]}"
  end
  ...
  defp do_database_server_url(db_props) do
    "#{db_props[:protocol]}://#{db_props[:user]}:#{db_props[:password]}@#{db_props[:hostname]}:#{
      db_props[:port]
    }"
  end
  ...
  def query_path(view_base_url, key, stale) when is_integer(key) do
    "#{view_base_url}?key=#{URI.encode_www_form(Integer.to_string(key))}&stale=#{
      Atom.to_string(stale)
    }"
  end
